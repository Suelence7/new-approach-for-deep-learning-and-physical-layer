""" IN this script the main of it is to create and define the source alphabet
THe source files used for either continous or discrete modulation in the source_material folder
For analog modulation : use of file serial-s01-e01.mp3
For digital modulation : use of gutenberg_shakespeare.txt
Packages to import : gnuradio, mediatools
For the mediatools installation a special read.me was created for that purpose since many bugs and errors
can be easily created if the tutorials found online were followed, so an effort in this part was made
significantly for making tasks easy for other perspectives
THis scripts corresponds to the part source alphabet in O'shea's paper 2016: RML datasets generation with GNU Radio
"""
...
class source_alphabet(gr.hier_block2):
    def __init__(self, dtype="discrete", limit=10000, randomize=False):
        if(dtype == "discrete"):
            gr.hier_block2.__init__(self, "source_alphabet", gr.io_signature(0, 0, 0), gr.io_signature(1, 1, gr.sizeof_char))
...
if __name__ == "__main__":
    ...
    tb = gr.top_block()
    src1 = source_alphabet("continuous", 1000)
