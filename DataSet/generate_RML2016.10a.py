    ...
    for alphabet_type in transmitters.keys():
        for i, mod_type in enumerate(transmitters[alphabet_type]):
            ...
            while insufficient_modsnr_vectors:
                ...
                mod = mod_type()
                # the parameters to choose and define their values depending on the channel conditions
                fD = 1
                delays = [0.0, 0.9, 1.7]
                mags = [1, 0.8, 0.3]
                ntaps = 8
                ...
                # when channel is applied we connect all the blocks : source, modulation, channel and snk
                if apply_channel:
                    tb.connect(src, mod, chan, snk)
                # if None channel is applied, we connect all the blocks without channel
                else:
...
pickle.dump(dataset, file("RML2018.08a_dict.dat", "wb"))
